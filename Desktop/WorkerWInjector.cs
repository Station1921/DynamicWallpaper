...
namespace DynamicWallpaper.Desktop
{
    ...
    public static class WorkerWInjector
    {
        ...
        public static void Attach(IntPtr childHwnd, IntPtr parentHwnd, Rectangle bounds)
        {
            ...
            if (raisedDesktop)
            {
                // Win11 24H2+ raised desktop 分支：必须反其道设置 WS_EX_LAYERED。
                // 微软 raised desktop 兼容方案要求挂到 Progman 的壁纸子窗口必须是 Layered 窗口，
                // 否则 DWM 不合成其内容到桌面（表现为"程序报已应用、桌面仍是静态壁纸"）。
                // 设完 Layered 后必须 SetLayeredWindowAttributes(alpha=255) 置满不透明，
                // 否则默认 alpha=0 窗口完全透明、同样不显示。
            ...
            {
                // 传统模式（Win10 经典 WorkerW / 非 raised）：绝不能设置 WS_EX_LAYERED。
                // 该结论仅适用于传统模式——WPF MediaElement 的视频内容走 DirectX/DirectComposition，
                // Layered 窗口不支持此类硬件加速内容参与 DWM 屏幕合成——PrintWindow 能抓到画面，
                // 但真实桌面始终只显示底层系统壁纸（表现为"程序报已应用、桌面无变化"）。
                exStyle = (exStyle | Win32.WS_EX_NOACTIVATE | Win32.WS_EX_TOOLWINDOW)
                          & ~Win32.WS_EX_APPWINDOW;
                Win32.SetWindowLong(childHwnd, Win32.GWL_EXSTYLE, exStyle);
                Logger.Log("[WorkerW] 窗口样式已设置（传统模式：非 Layered，保证 MediaElement 内容可合成）");
            }
